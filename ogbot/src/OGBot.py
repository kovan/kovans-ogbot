...
class Bot(threading.Thread):
    ...
    def _start(self): 
             ...
             # spy new planets
             newPlanets = [planet for planet in inactivePlanets if len(planet.spyReportHistory) == 0]
             self._spyPlanets(newPlanets,probesToSend)
                               
             # restore old simulations and initialize new planet's simulations
             ...
             del(loadedPlanets,newPlanets,inactivePlanets)
             ...
             while True:
                      ...
                      if len(self.simulations) > 0: # launch attacking espionages/attacks
                               ...
                               for planet, rentability in planets:
                                       ...
                                       print defendedStr, planet, rentability, self.simulations[planet].simulatedResources, "defenses:",
                               ...
                               try:                                
                                        if targetPlanet.spyReportHistory[-1].getAge(self.serverTime()).seconds < 600:
                                            ...
                                            simulation.simulatedResources -= resourcesToSteal # TODO: could -= operator be used here?
                                        ...
                                        self.simulations.remove(targetPlanet)                                  
    ...
    def _spyPlanets(self,planets,probesToSend):
             ...
             while len(notArrivedEspionages) or len(planets):
                      ...
                      if len(notArrivedEspionages):
                               displayedReports = self._web.getSpyReports()
                               for planet,espionage in notArrivedEspionages.items():
                                        expired = self.serverTime() > espionage.launchTime + timedelta(minutes=10)
                                        spyReport = self._didEspionageArrive(espionage,displayedReports)
                                        if  spyReport:
                                            del notArrivedEspionages[planet]
                                            validPlanets.append(planet)
                                            planet.spyReportHistory.append(spyReport)
                                        elif expired:    
                                            del notArrivedEspionages[planet]
